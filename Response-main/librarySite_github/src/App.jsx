    ...
    <div className="app">
      ...
      <header className="header">
        <div className="header-content">
          ...
          <nav className="nav-links">
            <button className="nav-item" title="나의 공간" aria-label="나의 공간 바로가기" onClick={() => setShowLoginModal(true)}>
            ...
            <button className="nav-item" title="로그인" aria-label="로그인 페이지로 이동" onClick={() => setShowLoginModal(true)}>
              <div className="icon-placeholder">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className="lecture-overlay">
          <div className="lecture-detail-content">
            ...
            <div className="detail-footer">
              <button className="apply-btn" onClick={() => setShowApplyComplete(true)}>수강 신청하기</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showApplyComplete && (
        <div className="modal-overlay">
          <div className="modal-content">
            <h3>접수 완료</h3>
            <p><strong>{selectedLecture?.title}</strong> 강좌 수강 신청이 정상적으로 완료되었습니다.</p>
            <div className="modal-actions" style={{ justifyContent: 'center' }}>
              <button className="btn-primary" onClick={() => {
                setShowApplyComplete(false);
        ...
        <div className="modal-overlay">
          <div className="modal-content" style={{ maxWidth: '400px', padding: '32px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
              <h3 style={{ margin: 0, fontSize: '24px', fontWeight: '700' }}>로그인</h3>
              <button style={{ background: 'none', border: 'none', fontSize: '24px', cursor: 'pointer', color: '#666', padding: 0 }} onClick={() => setShowLoginModal(false)} aria-label="닫기">✕</button>
            </div>
            
            <form onSubmit={(e) => { e.preventDefault(); alert('데모 버전에서는 로그인이 지원되지 않습니다.'); }}>
              <div style={{ marginBottom: '16px', textAlign: 'left' }}>
                <label style={{ display: 'block', fontSize: '14px', marginBottom: '8px', color: '#555', fontWeight: '600' }}>아이디</label>
                <input type="text" placeholder="아이디를 입력하세요" style={{ width: '100%', padding: '14px', border: '1px solid #ddd', borderRadius: '12px', fontSize: '15px', outline: 'none', boxSizing: 'border-box' }} />
              </div>
              <div style={{ marginBottom: '24px', textAlign: 'left' }}>
                <label style={{ display: 'block', fontSize: '14px', marginBottom: '8px', color: '#555', fontWeight: '600' }}>비밀번호</label>
                <input type="password" placeholder="비밀번호를 입력하세요" style={{ width: '100%', padding: '14px', border: '1px solid #ddd', borderRadius: '12px', fontSize: '15px', outline: 'none', boxSizing: 'border-box' }} />
              </div>
              
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <button type="submit" className="btn-primary" style={{ width: '100%', padding: '16px', borderRadius: '12px', fontSize: '16px', fontWeight: '700' }}>로그인</button>
                <button type="button" className="btn-secondary" style={{ width: '100%', padding: '16px', borderRadius: '12px', fontSize: '16px', fontWeight: '700' }} onClick={() => alert('데모 버전에서는 회원가입이 지원되지 않습니다.')}>회원가입</button>
              </div>
            </form>
            
            <div style={{ marginTop: '24px', textAlign: 'center', fontSize: '13px', color: '#888' }}>
              <span style={{ cursor: 'pointer' }} onClick={() => alert('준비 중입니다.')}>아이디 찾기</span>
              <span style={{ margin: '0 12px', color: '#ddd' }}>|</span>
              <span style={{ cursor: 'pointer' }} onClick={() => alert('준비 중입니다.')}>비밀번호 찾기</span>
            </div>
          </div>
        </div>
      )}
